...
RECOG_WEIGHTS = "rfdetr/checkpoints_recog_detector_crops/best_recognizer_detector_crops.pth"
...
def decode_text_current(pred_logits, max_chars=None, expected_chars=None):
...
class PlateTracker:
    def __init__(self, max_history=8, iou_thresh=0.3, recency_decay=0.9, min_obs_score=0.55, max_missed=6, max_chars=None, expected_chars=None):
        self.tracks = []
    ...
    def _stabilize_text(self, track):
        ...
        text = "".join(chars)
        conf = sum(char_scores) / len(char_scores) if char_scores else 0.0
        return text, conf
    ...
    def update(self, detections):
        ...
        for track in self.tracks:
            ...
            if text:
                outputs.append((track["bbox"], text, score))
        return outputs


class HybridALPR:
    def __init__(self, det_weights, rec_weights, det_conf=0.5, rec_conf=0.55, max_chars=None, expected_chars=None, track_history=8):
        ...
        print("Loading current recognizer weights...")
        ...
        num_classes = ckpt_rec.get("num_classes", NUM_CLASSES) if isinstance(ckpt_rec, dict) else NUM_CLASSES
        max_seq_len = ckpt_rec.get("max_seq_len", MAX_SEQ_LEN) if isinstance(ckpt_rec, dict) else MAX_SEQ_LEN
    ...
    def process_frame(self, frame):
        ...
        for i in range(len(predictions.xyxy)):
            ...
            with torch.no_grad():
                ...
                text, text_conf = decode_text_current(pred_chars, max_chars=self.max_chars, expected_chars=self.expected_chars)
        ...
        for bbox, text, score in stabilized_results:
            ...
            frame = draw_text(frame, text, (x1, max(0, y1 - 30)), color=color)
        return frame
...
def run():
    ...
    parser.add_argument("--output", type=str, default="hasil_hotfix.mp4")
    ...
    parser.add_argument("--expected-chars", type=int, default=None, help="Gunakan hanya jika Anda yakin panjang plat target tetap")
    parser.add_argument("--track-history", type=int, default=8)
