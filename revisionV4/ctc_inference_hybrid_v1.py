...
from ctc_config import CROP_HEIGHT, CROP_WIDTH, DEVICE, NUM_CLASSES, ctc_greedy_decode, letterbox_plate_image, sanitize_plate_text
...
RECOG_WEIGHTS = "rfdetr/checkpoints_recog_ctc/best_recognizer_ctc.pth"
...
def draw_text(frame, text, pos, color=(0, 255, 0), size=24):
...
class HybridALPRCTC:
    ...
    def recognize_crop(self, crop_bgr: np.ndarray):
        crop_tensor = crop_to_tensor(crop_bgr).to(DEVICE)
        with torch.no_grad():
    ...
    def process_frame(self, frame):
        ...
        for i in range(len(predictions.xyxy)):
            ...
            if crop.size == 0:
                continue
            text, text_conf = self.recognize_crop(crop)
            detections.append({
                "bbox": [x1, y1, x2, y2],
                "text": text,
                "text_conf": text_conf,
...
def run():
    parser = argparse.ArgumentParser(description="Hybrid ALPR inference with transformer+CTC recognizer")
    parser.add_argument("--input", type=str, required=True, help="Path ke video atau gambar")
    parser.add_argument("--output", type=str, default="hasil_ctc.mp4", help="Path output file")
    ...
    parser.add_argument("--max-chars", type=int, default=None, help="Batas maksimum karakter setelah decode CTC")
    parser.add_argument("--expected-chars", type=int, default=None, help="Opsional: potong output ke panjang tertentu")
    parser.add_argument("--track-history", type=int, default=10)
    args = parser.parse_args()

    system = HybridALPRCTC(
